...
<script type="text/javascript" language="javascript">
	function shangPage1(){
		$.ajax({
			type:'post',
			url:encodeURI("goodsZr/noint_goodszr_indexGoodsZr?page=${page-1}"),
	...
	function jump1(){
		var jumppage1 = $("#ccc").val();
		var max = $("#max").val();
		if(isNaN(jumppage1)){
			alert("页码为数字！");
			return;
		}
		if(parseInt(jumppage1)<1){
			alert("您输入的页码不存在！");
			return;
		}
		if(parseInt(jumppage1)>parseInt(max)){
			alert("超过最大页数，最大页数为"+max);
			return;
		}
		$.ajax({
			type:'post',
			url:encodeURI("goodsZr/noint_goodszr_indexGoodsZr?page="+jumppage1),
			beforeSend:function(XMLHttpRequest){
...
<body>
	...
	<div
		...
		<div
			...
			<input id="ccc" style="width: 100px; height: 30px; border: 0px;" />
		</div>
