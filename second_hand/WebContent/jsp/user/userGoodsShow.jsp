...
function shangPage(){
	...
	alert(userId);
	location = 'order/noint_order_findEvaluateNotNullByUser?userId='+userId+'&page=${page-1}'; 
}
function nextPage(){
	var userId = $("#userId").val();
	alert(userId);
...
function jump() {
		...
		location = 'order/noint_order_findEvaluateNotNullByUser?userId='+userId+'&page='+jumppage+''; 
}
	...
	<div id="main"
		...
		<div id="left"
			...
			<center>
				<div
					...
					<!-- 卖家更多闲置在goodsZr -->
					<a
						href="goodsZr/noint_goodszr_toListByUserMore?userId=<s:property value="user.userId" />">该卖家更多闲置</a><br />
		...
		<div id="right" style="height: auto; width: 680px; float: right">
			<div id="title"
				...
				卖家已出售宝贝评价反馈 <img title="交易已完成"
					src="index_img/changeStatu_button1.png" width="20px" height="20px"
					style="margin-top: 5px;" />
			</div>
			<s:iterator value="orderList" var="order" status="sta">
				<div id="goods" style="height: 220px; width: 680px; float: right">
					<div
						...
						<a
							href="goodsZr/noint_goodszr_toDetailPage.action?id=${order.goodsZr.goodsZrId}"><img
							width="100%" height="180px"
							src="goodspic${fn:substringAfter(order.goodsPicOne, 'goodspic')}"></img></a>
					</div>
					...
					<div
						...
						<div
							...
							<s:property value="#order.goodsZr.goodsName" />
							<img title="交易完成" src="index_img/changeStatu_button1.png"
						...
						<span
							style="font-size: 20px; line-height: 35px; color: #e4393c; font-weight: bolder; margin-bottom: 10px;">￥价格<s:property
								value="#order.goodsZr.price " /></span> &nbsp;&nbsp; <font
							color="#e4393c"> <c:if test="${order.goodsZr.tradeType==0}">仅同校交易</c:if>
							<c:if test="${order.goodsZr.tradeType==1}">仅同校或同城交易</c:if> <c:if
								test="${order.goodsZr.tradeType==2}">全国范围可邮寄</c:if>
						</font><br />
						<div
							style="width: 400px; height: 60px; line-height: 30px; font-size: 15px; margin-bottom: 10px; color: red">
							商品描述：
							<s:property value="#order.goodsZr.goodsMsg" />
						</div>
						<div
							style="width: 300px; height: 25px; line-height: 30px; font-size: 15px; margin-bottom: 10px; float: left;">

							<c:if test="${order.goodsZr.talkPrice == '0' }">
            		可以小刀
            	</c:if>
							<c:if test="${order.goodsZr.talkPrice == '1' }">
            		不可小刀  
            	</c:if>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;预交易地点：
							<s:property value="#order.goodsZr.address" />
						</div>
						<a
							href="goodsZr/noint_goodszr_toDetailPage.action?id=${goodsZr.goodsZrId}"><div
								style="width: 150px; height: 25px; line-height: 30px; font-size: 15px; margin-bottom: 10px; float: right;">
								留言条数：
								<s:property value="#order.goodsZr.comCount" />
								&nbsp;&nbsp;&nbsp;赞(
								<s:property value="#order.goodsZr.praise" />
								)
							</div></a>
					</div>
				</div>
				...
				<span
					style="font-size: 16px; line-height: 26px; color: #e4393c; font-weight: bold; margin-left: 15px;">宝贝评价</span>
				<br />
				<div id="userImgLeft"
					style="width: 60px; hight: 80px; float: left; margin-bottom: 5px; margin-left: 20px;">
					<c:if test="${order.user.photoUrl != null }">
						<img width="60px" height="60px"
							src="userImg${fn:substringAfter(order.user.photoUrl, 'userImg')}" />
					</c:if>
					<c:if test="${order.user.photoUrl== null }">
						<img src="index_img/user1.png" />
					</c:if>
				...
				<div id="userInfo"
					style="width: 80px; hight: 80px; float: left; margin-top: 5px; margin-left: 10px;">
					<s:property value="#order.user.nickname" />
					<br />
					<br />
					<s:property value="#order.user.lv" />
				</div>
				<div id="evaluationCon"
					style="height: 40px; width: 470px; margin-left: 40px; padding-top: 20px; float: right;">
					<s:property value="#order.evaluate" />
