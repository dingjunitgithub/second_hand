...
function addAdmin(){
	...
	function shangPage(){
		$.ajax({
			type:'get',
			url:encodeURI("user/user_toAdminList.action?page=${page-1}"),
			beforeSend:function(XMLHttpRequest){
	...
	function jump(){
		...
		$.ajax({
			type:'get',
			url:encodeURI("user/user_toAdminList.action?page="+jumppage),
			beforeSend:function(XMLHttpRequest){
...
<!-- 管理员列表 -->
	<div
		...
		<div class="navChoose">管理员列表</div>
		<div
			style="width: 30px; height: 30px; float: right; margin: 15px; cursor: pointer;"
			onclick="addAdmin();">
