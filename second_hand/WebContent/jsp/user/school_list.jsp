...
<script type="text/javascript" language="javascript">
	function toadd(){
		$.ajax({
			type:'post',
			url:encodeURI("school/school_toAdd.action"),
	...
	function schooltoupdate(id){
		$.ajax({
			type:'get',
			url:encodeURI("school/school_toUpdate.action?schoolId="+id),
			beforeSend:function(XMLHttpRequest){
	...
	function schooltodelete(id){
		$.ajax({
			type:'get',
			url:encodeURI("school/school_delete.action?schoolId="+id),
			beforeSend:function(XMLHttpRequest){
	...
	function shangPage(){
		$.ajax({
			type:'get',
			url:encodeURI("school/school_toList.action?page=${page-1}"),
			beforeSend:function(XMLHttpRequest){
	...
	function jump(){
		...
		$.ajax({
			type:'get',
			url:encodeURI("school/school_toList.action?page="+jumppage),
			beforeSend:function(XMLHttpRequest){
                    ...
                    <c:forEach items="${schoolList}" var="school" varStatus="status">
                        ...
                        <td>
	                        <div style="width:20px; height:20px;float:left; cursor: pointer;margin-left:10px;">
	                        	 <img src="index_img/update_button.png" width="100%" height="100%"  onclick="schooltoupdate('${school.schoolId}');"/>
	                        </div>
	                         <div style="width:20px; height:20px;float:left;cursor: pointer;margin-left:10px;">
	                        	 <img src="index_img/delete_button.png" width="100%" height="100%"  onclick="schooltodelete('${school.schoolId}');"/>
	                        </div>
                        </td>
	                    </tr>
	                    <tr>
