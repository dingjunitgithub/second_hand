...
function shangPage(){
	$.ajax({
		type:'get',
		url:encodeURI("comment/comment_toListToMeNo.action?page=${page-1}"),
		beforeSend:function(XMLHttpRequest){
...
function jump(){
	...
	$.ajax({
		type:'get',
		url:encodeURI("comment/comment_toListToMeNo.action?page="+jumppage),
		beforeSend:function(XMLHttpRequest){
...
<!-- 留言未回复页面 -->
	<div
		...
		<div
			...
			<a onclick="chooseNo()"><img src="index_img/norevert.png" /><font
				color="#00bb00">待回复</font></a>

		</div>
		<div id="sendCount"
			style="width: 42px; height: 40px; position: absolute; left: 395px; top: 160px; background-image: url(index_img/save1.png); line-height: 32px; text-align: center; color: #ffffff; font-size: 16px; font-weight: bolder;">
			${waitRevertCount}</div>
		<div
			style="width: 40px; height: 36px; cursor: pointer; float: left; margin-left: 50px; margin-top: 5px;">
			<a onclick="chooseYes()"><img src="index_img/yesrevert.png" /><font
				color="#00bb00">已回复</font></a>
	...
	<div
		...
		<div
			style="width: 98%; margin: 10px auto; font-size: 20px; font-style: bolder; float: left; color: red;">
			待回复</div>
		<s:iterator value="commentList" var="comment" status="sta">
			...
			<div id="right"
				...
				<c:if test="${comment.autoType==0}">
					<div
						style="width: 40px; color: red; height: 10px; line-height: 30px; float: right; text-align: right; padding-right: 10px"
						onclick="open_revert_div(<s:property value='#comment.comId'/>,<s:property value='#comment.user.userId'/>,<s:property value='#comment.goodsZr.goodsZrId'/>,'${sta.count}')">
						
						回复</div>
				</c:if>
				<c:if test="${comment.autoType==1}">
				<div style="width: 80px; color: red; height: 10px; line-height: 30px; float: right; text-align: right; padding-right: 10px">
						提醒卖家发货
					</div>
				</c:if>
			</div>
