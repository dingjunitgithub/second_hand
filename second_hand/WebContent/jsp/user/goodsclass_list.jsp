...
<script type="text/javascript" language="javascript">
	function toadd(){
		$.ajax({
			type:'post',
			url:encodeURI("gClass/goodsclass_toAdd.action"),
	...
	function schooltoupdate(id){
		$.ajax({
			type:'get',
			url:encodeURI("gClass/goodsclass_toUpdate.action?id="+id),
			beforeSend:function(XMLHttpRequest){
	...
	function schooltodelete(id){
		$.ajax({
			type:'get',
			url:encodeURI("gClass/goodsclass_delete.action?id="+id),
			beforeSend:function(XMLHttpRequest){
	...
	function shangPage(){
		$.ajax({
			type:'get',
			url:encodeURI("gClass/goodsclass_toList.action?page=${page-1}"),
			beforeSend:function(XMLHttpRequest){
	...
	function jump(){
		...
		$.ajax({
			type:'get',
			url:encodeURI("gClass/goodsclass_toList.action?page="+jumppage),
			beforeSend:function(XMLHttpRequest){
			},
			success:function (data, textStatus){
				$("#content").html(data);
...
<body>
           ...
           <div style="width:98%; height:510px; margin:0px auto; background-color:#ffffff;">
           		<table width="100%" border="0">
                    ...
                    <c:forEach items="${goodsClassList}" var="goodsClass" varStatus="status">
                    	<tr height="30px" align="center" >
                    	<td>${status.count }</td>
                        <td>${goodsClass.className}</td>
                        <td>
                         <div style="width:20px; height:20px;float:left; cursor: pointer;margin-left:10px;">
	                        	 <img src="index_img/update_button.png" width="100%" height="100%"  onclick="schooltoupdate('${goodsClass.classId}');"/>
	                        </div>
	                         <div style="width:20px; height:20px;float:left;cursor: pointer;margin-left:10px;">
	                        	 <img src="index_img/delete_button.png" width="100%" height="100%"  onclick="schooltodelete('${goodsClass.classId}');"/>
	                        </div>
	                    </tr>
	                    <tr>
	                    	<td colspan="4">
